...
public class PurchaseAssembler {
    public <T extends BaseResponse> EntityModel<T> toModel(T entity, Authentication authentication) {
        EntityModel<T> entityModel = EntityModel.of(entity,
                ...
                WebMvcLinkBuilder.linkTo(WebMvcLinkBuilder.methodOn(PurchaseController.class).getAllTransactionsOfBuyer(entity.getId(), authentication)).withRel("Get all transactions of user").withType(HttpMethod.GET.name())
        );
