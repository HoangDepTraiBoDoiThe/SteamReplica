...
public class ServiceHelper {
    ...
    public <T extends BaseResponse> CollectionModel<EntityModel<T>> makeCategoryResponse_CollectionModel(Class<T> responseType, List<Category> categories, Authentication authentication) {
        try {
            List<T> responses = categories.stream().map(category -> {
                if (CategoryResponse_Full.class.equals(responseType)) return (T) new CategoryResponse_Full(category);
                else return (T) new CategoryResponse_Minimal(category);
            }).toList();
            return categoryAssembler.toCollectionModel(responses, authentication);
        } catch (Exception e) {
            throw new RuntimeException("Error while creating response: ", e);
        }        
    }
    ...
    public <T extends BaseResponse> CollectionModel<EntityModel<T>> makeDiscountResponse_CollectionModel(Class<T> responseType, List<Discount> discounts, Authentication authentication) {
        try {
            List<T> list = discounts.stream().map(discount -> {
                if (DiscountResponse_Full.class.equals(responseType)) {
                    return (T) new DiscountResponse_Full(discount);
                } else if (DiscountResponse_Basic.class.equals(responseType)) {
                    return (T) new DiscountResponse_Basic(discount);
                } else return (T) new DiscountResponse_Minimal(discount);
            }).toList();
            return discountAssembler.toCollectionModel(list, authentication);
        } catch (Exception e) {
            throw new RuntimeException("Error while creating response: ", e);
        }
    }
